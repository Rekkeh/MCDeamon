/*
	Copyright 2010 MCLawl team (Modified for use with MCDeamon) (Modified for use with MCDeamon) Licensed under the
        ...
        static void Save(string givenPath)
        {
            try
            {
                ...
                if (givenPath.IndexOf("server") != -1)
                {
                    ...
                    w.WriteLine("#   DatabaseName\t=\tThe name of the database stored (Default = MCDeamon)");
                    w.WriteLine();
                    ...
                    w.WriteLine("host-state = " + Server.DeamonState.ToString());
                    w.WriteLine();
