...
namespace MCDeamon_.Gui
{
    public static class Program
    {
        ...
        {
            if (Process.GetProcessesByName("MCDeamon").Length != 1)
            {
                foreach (Process pr in Process.GetProcessesByName("MCDeamon"))
                {
        ...
        remake:
            try
            {
                ...
                if (foundView[4].Split(' ')[2].ToLower() == "true")
                {
                    ...
                    Console.Title = Server.name + " MCDeamon Version: " + Server.Version;
                    usingConsole = true;
                ...
                else
                {
                    ...
                    Application.Run(new MCDeamon.Gui.Window());
                }
            }
        ...
        talk: handleComm("say " + MCDeamon.Group.findPerm(LevelPermission.Admin).color + "Console: &f" + s);
            handleComm(Console.ReadLine());
        }
        ...
        public static void UpdateCheck(bool wait = false, Player p = null)
        {
            ...
            Thread updateThread = new Thread(new ThreadStart(delegate
            {
                ...
                try
                {
                    if (Client.DownloadString("http://www.MCDeamon.tk/curversion.txt") != Server.Version)
                    {
                        ...
                        else
                        {
                            ...
                            else
                            {
                                ...
                                Console.WriteLine("Update using the file at www.MCDeamon.tk/MCDeamon_.dll and placing it over the top of your current MCDeamon_.dll!");
        ...
        public static void PerformUpdate(bool oldrevision)
        {
            try
            {
                ...
                if (!Server.mono)
                {
                    ...
                    SW.WriteLine("if exist MCDeamon_.dll.backup (erase MCDeamon_.dll.backup)");
                    SW.WriteLine("if exist MCDeamon_.dll (rename MCDeamon_.dll MCDeamon_.dll.backup)");
                    SW.WriteLine("if exist MCDeamon.new (rename MCDeamon.new MCDeamon_.dll)");
                    SW.WriteLine("start MCDeamon.exe");
                }
                else
                {
                    ...
                    SW.WriteLine("kill $2");
                    SW.WriteLine("rm MCDeamon_.dll.backup");
                    SW.WriteLine("mv MCDeamon_.dll MCDeamon.dll_.backup");
                    SW.WriteLine("wget http://MCDeamon.tk/MCDeamon_.dll");
                    SW.WriteLine("mono MCDeamon.exe");
                }
                ...
                {
                    WebClient client = new WebClient();
                    Server.selectedrevision = client.DownloadString("http://www.MCDeamon.tk/curversion.txt");
                    client.Dispose();
                }
                verscheck = Server.selectedrevision.TrimStart('r');
                int vers = int.Parse(verscheck.Split('.')[0]);
                if (oldrevision) { filelocation = ("http://www.MCDeamon.tk/archives/exe/" + Server.selectedrevision + ".exe"); }
                if (!oldrevision) { filelocation = ("http://www.MCDeamon.tk/MCDeamon_.dll"); }
                WebClient Client = new WebClient();
                Client.DownloadFile(filelocation, "MCDeamon.new");
                Client.DownloadFile("http://www.MCDeamon.tk/changelog.txt", "extra/Changelog.txt");
                ...
                try
                {
                    if (MCDeamon.Gui.Window.thisWindow.notifyIcon1 != null)
                    {
                        MCDeamon.Gui.Window.thisWindow.notifyIcon1.Icon = null;
                        MCDeamon.Gui.Window.thisWindow.notifyIcon1.Visible = false;
                    }
                }
                catch { }
